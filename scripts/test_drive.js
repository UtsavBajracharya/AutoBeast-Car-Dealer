function validateTestDriveForm() {
    const form = document.getElementById('testdrive_myForm');

    // Validate first name
    const firstNameInput = document.getElementById('td_firstName');
    ...
    // Validate last name
    const lastNameInput = document.getElementById('td_lastName');
    ...
    // Validate email
    const email = document.getElementById('td_email').value.trim();
    ...
    const phoneNumber = document.getElementById('td_phoneNumber').value.trim();
    ...
    var inputDate = document.getElementById('td_date').value;
    var today = new Date().toISOString().split('T')[0]; // Get today's date in YYYY-MM-DD format

    if (inputDate < today) {
        alert("Please select a future date for the test drive.");
        return false; // Prevent form submission
    }
